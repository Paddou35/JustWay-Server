	...
	if xPlayer.job.name == society.name then
		TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
			if amount > 0 and account.money >= amount then
                ...
                TriggerClientEvent('okokNotify:Alert', xPlayer, "Information", _U('have_withdrawn', ESX.Math.GroupDigits(amount)), 4000, 'info')
			else
                TriggerClientEvent('okokNotify:Alert', xPlayer, "Information", _U('invalid_amount'), 4000, 'info')
			end
		end)
	else
	...
	if xPlayer.job.name == society.name then
		if amount > 0 and xPlayer.getMoney() >= amount then
			TriggerEvent('esx_addonaccount:getSharedAccount', society.account, function(account)
                ...
                TriggerClientEvent('okokNotify:Alert', xPlayer, "Information", _U('have_deposited', ESX.Math.GroupDigits(amount)), 4000, 'info')
	...
	if xPlayer.job.name == society then
		if amount and amount > 0 and account.money >= amount then
			...
			function(rowsChanged)
                TriggerClientEvent('okokNotify:Alert', xPlayer, "Information", _U('you_have', ESX.Math.GroupDigits(amount)), 4000, 'info')
			end)
		else
            TriggerClientEvent('okokNotify:Alert', xPlayer, "Information", _U('invalid_amount'), 4000, 'info')
		end
...
ESX.RegisterServerCallback('esx_society:setJob', function(source, cb, identifier, job, grade, type)
	...
	if isBoss then
		...
		if xTarget then
			...
			if type == 'hire' then
                TriggerClientEvent('okokNotify:Alert', xTarget, "Information", _U('you_have_been_hired', job), 4000, 'info')
			elseif type == 'promote' then
                TriggerClientEvent('okokNotify:Alert', xTarget, "Information", _U('you_have_been_promoted'), 4000, 'info')
			elseif type == 'fire' then
                TriggerClientEvent('okokNotify:Alert', xTarget, "Information", _U('you_have_been_fired', xTarget.getJob().label), 4000, 'info')
			end
...
function WashMoneyCRON(d, h, m)
	MySQL.query('SELECT * FROM society_moneywash', function(result)
		for i=1, #result, 1 do
			...
			if xPlayer then
                TriggerClientEvent('okokNotify:Alert', xPlayer, "Information", _U('you_have_laundered', ESX.Math.GroupDigits(result[i].amount)), 4000, 'info')
			end
