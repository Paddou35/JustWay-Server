			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boss_actions_' .. society, {
				...
				if data.current.value == 'withdraw_society_money' then
					ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'withdraw_society_money_amount_' .. society, {
						...
						if amount == nil then
                            exports['okokNotify']:Alert("Information", _U('invalid_amount'), 4000, 'info')
						else
							menu2.close()
	...
	ESX.TriggerServerCallback('esx_society:getEmployees', function(employees)
		...
		ESX.UI.Menu.Open('list', GetCurrentResourceName(), 'employee_list_' .. society, elements, function(data, menu)
			...
			elseif data.value == 'fire' then
                exports['okokNotify']:Alert("Information", _U('you_have_fired', employee.name), 4000, 'info')
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_' .. society, {
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'recruit_confirm_' .. society, {
				...
				if data2.current.value == 'yes' then
                    exports['okokNotify']:Alert("Information", _U('you_have_hired', data.current.name), 4000, 'info')
...
function OpenPromoteMenu(society, employee)
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'promote_employee_' .. society, {
			...
			menu.close()
            exports['okokNotify']:Alert("Information", _U('you_have_promoted', employee.name, data.current.label), 4000, 'info')
...
function OpenManageGradesMenu(society)
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
		local elements = {}
		if job.name == "ambulance" or job.name == "police" or job.name == "bcso" then
            exports['okokNotify']:Alert("Information", "Vous ne pouvez pas gérer les salaires de votre service public", 4000, 'info')
        else
            for i=1, #job.grades, 1 do
                local gradeLabel = (job.grades[i].label == '' and job.label or job.grades[i].label)
            ...
            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'manage_grades_' .. society, {
                ...
                ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'manage_grades_amount_' .. society, {
                    ...
                    elseif amount > Config.MaxSalary then
                        exports['okokNotify']:Alert("Information", _U('invalid_amount_max'), 4000, 'info')
                    elseif amount < Config.MinSalary then
                        exports['okokNotify']:Alert("Information", _U('invalid_amount_min'), 4000, 'info')
                    else
                        menu2.close()
...
RegisterNetEvent('esx_society:bossmenu')
AddEventHandler('esx_society:bossmenu', function()
	ESX.TriggerServerCallback('esx_society:getJob', function(job)
