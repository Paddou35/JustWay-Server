		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boat_shop_confirm', {
			...
			if data2.current.value == 'yes' then
				...
				ESX.TriggerServerCallback('esx_boat:buyBoat', function(bought)
					if bought then
                        exports['okokNotify']:Alert("Information", _U('boat_shop_bought', data.current.name, ESX.Math.GroupDigits(data.current.price)), 4000, 'info')
                        ...
                        exports['okokNotify']:Alert("Information", _U('boat_shop_nomoney'), 4000, 'info')
						menu2.close()
					end
				end, props)
			else
...
function OpenBoatGarage(garage)
	ESX.TriggerServerCallback('esx_boat:getGarage', function (ownedBoats)
		if #ownedBoats == 0 then
			exports['okokNotify']:Alert("Information", _U('garage_noboats'), 4000, 'info')
		else
			...
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'boat_garage', {
				...
				if ESX.Game.IsSpawnPointClear(garage.SpawnPoint, 4.0) then
                    ...
                    exports['okokNotify']:Alert("Information", _U('garage_taken'), 4000, 'info')
                    ...
                    exports['okokNotify']:Alert("Information", _U('garage_blocked'), 4000, 'info')
				end
			end, function (data, menu)
				menu.close()
		...
		if data.current.value == 'yes' then
			ESX.TriggerServerCallback('esx_boat:buyBoatLicense', function (boughtLicense)
				if boughtLicense then
                    exports['okokNotify']:Alert("Information", _U('license_bought', ESX.Math.GroupDigits(Config.LicensePrice)), 4000, 'info')
					menu.close()

					OpenBoatShop(shop) -- parse current shop
				else
                    exports['okokNotify']:Alert("Information", _U('license_nomoney'), 4000, 'info')
				end
			end)
		else
			CurrentAction    = 'boat_shop'
...
function StoreBoatInGarage(vehicle, teleportCoords)
	...
	ESX.TriggerServerCallback('esx_boat:storeVehicle', function (rowsChanged)
		if rowsChanged > 0 then
            ...
            exports['okokNotify']:Alert("Information", _U('garage_stored'), 4000, 'info')
			local playerPed = PlayerPedId()
            ...
            exports['okokNotify']:Alert("Information", _U('garage_notowner'), 4000, 'info')
		end
